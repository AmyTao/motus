...
<Tip>
  Add type annotations to your parameters. Motus builds the JSON Schema from them, and any parameter without a hint is dropped from what the model sees. A parameter that has a default value will still work (the model just cannot override it, so the default is always used); a parameter with no default will crash the tool at call time with a missing-argument error. Annotations are not enforced at registration, so a broken tool registers silently and only fails when the agent invokes it.
</Tip>
