...
<Tip>
  Add type annotations to your parameters. Motus turns them into the JSON Schema the model uses to call your tool. Any parameter without a type hint is silently left out of the schema, so the model has no way to know about it or pass a value for it. The function will still work as a tool, but only its annotated parameters are reachable from the model side.
</Tip>
